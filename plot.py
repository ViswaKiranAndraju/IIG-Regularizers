...
GAME_SCALES = {'kuhn_poker': 4.0, 'leduc_poker': 26.0}
...
if __name__ == '__main__':
    ...
    # Get the number of game names
    num_games = len(exp_generator.game_names)

    # Create a list to hold axes objects
    axs = []

    # Create subplots and append axes objects to the list
    for _ in range(num_games):
        fig, ax = plt.subplots()
        axs.append(ax)

    for i, game_name in enumerate(exp_generator.game_names):
        y_label = 'Regret' if i == 0 else ''
        df = make_df('average', game_name, results, exp_generator)
        plot_ax(axs[i], df, 'average', y_label)
        axs[i].set_title(f'Average regret. in {game_name}'.replace('_', ' ').replace('kuhn', 'Kuhn').replace('leduc', 'Leduc'))

    save_path = os.path.join(exp_generator.save_path, exp_generator.description+'_plot_regret.pdf')
